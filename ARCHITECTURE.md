...
2. For each row, open a `SAVEPOINT`, then dispatch by `target_kind`:
   ...
   - `workflow` → API-rejected with 400 until the workflow runtime exists.
3. Compute next `next_fire_at` (croner advances past `now` in the schedule's timezone). One-shots whose `fire_at` is now past are `DELETE`d. If `catchup: true`, missed occurrences between the original `next_fire_at` and `now` are fired in order, bounded by `catchup_limit`.
4. `UPDATE` the row, `RELEASE SAVEPOINT`. Per-row failures `ROLLBACK TO SAVEPOINT`, increment `consecutive_failures`, and pause the schedule at `failure_threshold` — all without losing the other rows in the batch.
5. `COMMIT`. All produced messages and all advances flush together; receivers wake via the same XactCallback path producers use.

Every fired message carries `headers._schedule = { name, scheduled_for, out_of_band }` so the target queue's archive is the audit log — queryable by `headers->'_schedule'->>'name'`. No fire log table.
...
| `src/routes/schedules.rs`               | `POST/GET /v1/schedules`, `GET/PUT/PATCH/DELETE /v1/schedules/{name}`, `POST /v1/schedules/{name}/runs`.                                                                                                                                                                                                                                    |
| `src/routes/previews.rs`                | `POST /v1/previews` — dry-run a schedule expression.                                                                                                                                                                                                                                                                                        |
| `src/schedule/`                         | Expression layer: `cron.rs` wraps `croner`; `every.rs` and `humanize.rs` translate shorthand/natural-language to canonical cron; `expression.rs` is the pure Expression → Canonical reduction.                                                                                                                                              |
| `src/ops/schedule.rs`                   | Schedule CRUD ops: `create` (strict), `upsert` (idempotent), `list`, `get`, `patch`, `delete`, `run_now`, `preview`. Validation, `_schedule` header merge, dispatch helper.                                                                                                                                                                 |
| `src/ops/schedule_worker.rs`            | Tokio task: claim due rows with `FOR UPDATE SKIP LOCKED`, savepoint-per-row dispatch via `queue.send` / `queue.publish_event`, advance / delete one-shots, record per-row failures, pause at `failure_threshold`.                                                                                                                           |
...
| `beyond-queue-extension/sql/schema.sql` | DDL for `queue.meta`, `queue.q_{name}`, `queue.a_{name}`, `queue.topic_subscriptions`, `queue.http_deliveries`, `queue.notify_insert_throttle`, `queue.schedule` (+ `schedule_status` / `schedule_target_kind` enums). PL/pgSQL functions: `receive_fifo`, FIFO grouped reads, topic routing, `queue_http_deliveries`, notification system. |
...
| `DELETE` | `/v1/schedules/{name}`                    | Idempotent delete. Returns 204.                                                                                                       |
| `POST`   | `/v1/schedules/{name}/runs`               | Manual run, out-of-band. 202 with a `Run` resource carrying `msg_ids`. Does not advance `next_fire_at`.                               |
| `POST`   | `/v1/previews`                            | Dry-run any expression. Returns canonical cron + `human_readable` + `next_fires`. No persistence.                                     |
